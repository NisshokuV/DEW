...
function pilotsOne()
  {
    let newPilots = pilots.map(pilot => `${pilot.name}, experiencia: ${pilot.years}`);
    return newPilots;
  }
function pilotsTwo()
{
  let newPilots = pilots.map(pilot => (
    {
      name: pilot.name,
      years: pilot.years
    }
...
function pilotsThree()
{
  let newPilots = pilots.filter(pilot => pilot.years > 16);
  return newPilots;
}
function pilotsFour()
{
  let totalExp = pilots.reduce((total, pilot) => total + pilot.years, 0);
  return totalExp;
}
function pilotsFive()
{
  let newPilots = pilots.filter(pilot => pilot.years > 15).map(pilot => ({
    id: pilot.id,
    name: pilot.name
  }));
  return newPilots;
...
function pilotsSix()
{
  let newPilots = pilots.map(pilot => ({
    id: pilot.id,
    iniciales: pilot.name.split(' ').map(n => n[0]).join('')
  }));
  return newPilots;
}
function pilotsSeven()
{
  let newPilots = pilots.reduce((acc, pilot) => 
    {
    
      let initials = pilot.name.split(' ').reduce((initials, word) => initials + word[0], '');
...
function municipiosOne()
{
  let result = municipios.filter(municipio => municipio.poblacion < 2000);
  return result;
}
function municipiosTwo()
{
  let result = municipios.filter(municipio => municipio.poblacion < 2000)
    .map(municipio => ({municipio: municipio.municipio, poblacion: municipio.poblacion }));
  return result;
}
function municipiosThree()
{
  let result = municipios.filter(municipio => municipio.isla === 'La Palma')
  .reduce((total,municipio) => total + municipio.poblacion,0);
  return result;
}
function municipiosFour()
{
 let result = 
   municipios.map(municipio => `${municipio.municipio}, poblacion: ${municipio.poblacion}`);
  return result;
}
function municipiosFive()
{
  let result = municipios.filter(municipio => municipio.isla === 'La Gomera')
  .map(municipio => ({municipio: municipio.municipio, poblacion: municipio.poblacion}));
  return result;
}